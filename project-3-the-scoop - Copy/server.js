...
function updateComment(url, request) {
  //locate id
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //save comment id to variable
  const savedComment = database.comments[id];
  //updated body passed through request
  const newComment = request.body && request.body.comment;
  const response = {};

//if no id or comment update supplied
  if (!id || !newComment) {
    response.status = 400;
//if comment does not exist
    ...
    savedComment.url = newComment.url || savedComment.url;
...
function deleteComment(url, request) {
  //locating id
  const id = Number(url.split('/').filter(segment => segment)[1]);
  //saving comment and article to variables
  const savedComment = database.comments[id];
  const savedArticle = database.articles[id];
  const response = {};
//if the article and the comment exist
  if (savedComment && savedArticle) {
    //delete the comment
...
function downvoteComment(url, request) {
  ...
  const username = request.body && request.body.username;
  //assigning article and comment to variables
  let savedComment = database.comments[id];
  const response = {};
//checking that comment, user exist
  if (savedComment && database.users[username]) {
    if (savedComment.downvotedBy.includes(username)) {
      savedComment.downvotedBy.splice(savedComment.downvotedBy.indexOf(username), 1);
    } if (!savedComment.downvotedBy.includes(username)) {
